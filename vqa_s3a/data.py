...
def get_reduced_loader(reduced_path, train=False, val=False):
    assert train + val == 1
    split = ReducedVQA(
        reduced_path,
        utils.path_for(train=train, val=val, test=False, question=True),
        utils.path_for(train=train, val=val, test=False, answer=True),
        config.preprocessed_path,
        answerable_only=train,
    )
    loader = torch.utils.data.DataLoader(
...
class ReducedVQA(data.Dataset):
    """ VQA dataset, open-ended """
    def __init__(self, reduced_path, questions_path, answers_path, image_features_path, answerable_only=False):
        super(ReducedVQA, self).__init__()
        self.answerable_only = answerable_only
        self.vqa = VQA(questions_path, answers_path, image_features_path, answerable_only)
        with open(reduced_path, 'rb') as f:
            reduced_ckp = pickle.load(f)
        self.reduced_questions = [] # idx, question
        self._max_length = 0
        for x in tqdm(reduced_ckp):
            if answerable_only:
                if x['idx'] not in self.vqa.answerable:
                    continue
                idx = self.vqa.answerable.index(x['idx'])
            else:
                if x['idx'] > len(self.vqa):
                    continue
                idx = x['idx']
            # for r in x['reduced']:
            r = x['reduced'][0]
            if len(r) == 0:
                continue
            self.reduced_questions.append((idx, torch.LongTensor(r)))
            self._max_length = max(len(r), self._max_length)
            if len(self.reduced_questions) > 200000:
                break
    ...
    @property
    def num_tokens(self):
        return self.vqa.num_tokens

    def __getitem__(self, item):
        item = item % len(self.reduced_questions)
        idx, reduced_q = self.reduced_questions[item]
        q_len = reduced_q.shape[0]
        if reduced_q.shape[0] < self.max_question_length:
            len_diff = self.max_question_length - reduced_q.shape[0]
            zeros = torch.LongTensor([0 for _ in range(len_diff)])
            reduced_q = torch.cat([reduced_q, zeros], 0)
        v, q, a, item, q_length = self.vqa[idx]
        return v, reduced_q, a, item, reduced_q.shape[0]

    def __len__(self):
        return len(self.reduced_questions)
