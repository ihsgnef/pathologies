...
class Net(nn.Module):
    ...
    def forward(self, v, q, q_len, embed_grad_hook=None):
        q = self.text(q, list(q_len.data), embed_grad_hook)
...
class TextProcessor(nn.Module):
    ...
    def forward(self, q, q_len, embed_grad_hook=None):
        embedded = self.embedding(q)
        if embed_grad_hook is not None:
            embedded.register_hook(embed_grad_hook)
